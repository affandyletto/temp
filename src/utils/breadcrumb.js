...
export function generateBreadcrumbOneSnap(pathname, getLabelById) {
  const segments = pathname.split("/").filter(Boolean);
  const oneSnapParents = ["onesnaps", "projects", "clients"]; // Added onesnaps as a parent
  const items = [{ label: "Home", path: null }];
  let currentPath = "";
  ...
  segments.forEach((segment, index) => {
    ...
    const isPreviousOneSnapParent = oneSnapParents.includes(segments[index - 1]);
    ...
    if (isLast) {
      ...
      // Handle specific OneSnap route naming
      if (segment === "onesnaps") {
        label = "OneSnaps";
      } else {
        label = segment
